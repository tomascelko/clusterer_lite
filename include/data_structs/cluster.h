...
template <typename data_type> class cluster
{
protected:
...
public:
  ...
  bool is_valid() const { return hits_.size() > 0; }

  virtual ~cluster() = default;

  double first_toa() const { return first_toa_; }

  double last_toa() const { return last_toa_; }

  /*uint64_t line_start() const
  {
      return line_start_;
  }*/
  uint64_t hit_count() const { return hits().size(); }

  /*uint64_t byte_start() const
  {
      return byte_start_;
  }*/
  std::vector<data_type> &hits() { return hits_; }

  const std::vector<data_type> &hits() const { return hits_; }
  ...
  void set_first_toa(double toa) { first_toa_ = toa; }

  void set_last_toa(double toa) { last_toa_ = toa; }

  uint64_t size()
  {
    return hits_.size() * data_type::avg_size() + 2 * sizeof(double) +
           3 * sizeof(uint64_t);
  }
  ...
  /*virtual void write(std::ofstream* cl_stream, std::ofstream* px_stream) const
  {
      ...
      *px_stream << std::fixed << std::setprecision(6) << first_toa_ << " " <<
  hit_count_<< std::string(" ") << line_start_ << " " << byte_start_ <<
  std::endl; for (uint32_t i = 0; i < hits_.size(); i++)
      {
          *px_stream << hits_[i];
      }
      *px_stream << "#" << std::endl;
  ...
  double time() const { return first_toa(); }

  void temporal_sort()
  {
    std::sort(hits_.begin(), hits_.end(),
              [](const auto &left_hit, const auto &right_hit)
  ...
  virtual std::pair<double, double> center()
  {
    ...
    return std::make_pair<double, double>(mean_x / hits_.size(),
                                          mean_y / hits_.size());
  }

  static constexpr uint64_t avg_size() { return 20 * data_type::avg_size(); }

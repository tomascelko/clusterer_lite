...
private:
  ...
  using optional_clusters = std::optional<std::vector<mm_hit, std::allocator<mm_hit>>>;
...
protected:
  ...
  void merge_clusters(unfinished_cluster<mm_hit> &base_cluster, unfinished_cluster<mm_hit> &new_cluster)
  ...
  // try always merging to left (smaller toa)
  {
    ...
    base_cluster.cl.hits().reserve(base_cluster.cl.hits().size() + new_cluster.cl.hits().size());
    base_cluster.cl.hits().insert(base_cluster.cl.hits().end(), std::make_move_iterator(new_cluster.cl.hits().begin()),
                                  std::make_move_iterator(new_cluster.cl.hits().end())); // TODO try hits in std list and then concat in O(1)

    // merge clusters
    base_cluster.pixel_iterators.reserve(base_cluster.pixel_iterators.size() + new_cluster.pixel_iterators.size());
    base_cluster.pixel_iterators.insert(base_cluster.pixel_iterators.end(), std::make_move_iterator(new_cluster.pixel_iterators.begin()),
                                        std::make_move_iterator(new_cluster.pixel_iterators.end())); // TODO try hits in std list and then concat in O(1)

    // update first and last toa
    base_cluster.cl.set_first_toa(std::min(base_cluster.cl.first_toa(), new_cluster.cl.first_toa()));
    base_cluster.cl.set_last_toa(std::max(base_cluster.cl.last_toa(), new_cluster.cl.last_toa()));

    unfinished_clusters_.erase(new_cluster.self);
    --unfinished_clusters_count_;
  }

  std::vector<cluster_it> find_neighboring_clusters(const coord &base_coord, double toa, cluster_it &oldest_cluster)
  {
    ...
    for (auto neighbor_offset : EIGHT_NEIGHBORS) // check all neighbor indexes
    {
      ...
      {
        if (std::abs(toa - neighbor_cl_it->cl.last_toa()) < cluster_diff_dt && !neighbor_cl_it->selected)
...
public:
  ...
  std::vector<cluster<mm_hit>> get_old_clusters(std::vector<cluster<mm_hit>> &old_clusters, double hit_toa = 0)
  {
  ...
  std::vector<cluster<mm_hit>> process_hits(hit_vect_iterator first, hit_vect_iterator last)
  {

...
coord::coord() {}
coord::coord(short x, short y) : x_(x),
                                 y_(y) {}
short coord::x() const
{
  return x_;
...
int32_t coord::linearize(uint32_t tile_size) const // use if untiled coord -> tiled linear
{
  return (MAX_VALUE / tile_size) * (x_ / tile_size) + (y_ / tile_size);
}
int32_t coord::linearize_tiled(uint32_t tile_size) const // use if tiled coord -> tiled linear
{
